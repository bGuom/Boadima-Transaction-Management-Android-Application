...
public class RemoveMembersActivity extends Activity {
    ...
    public class CustomAdapter extends BaseAdapter {
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            MemID = UniqueList.get(i);

            bT.setText("ADD");
            UserLogo.setImageResource(R.drawable.user);
            UserName.setText(Fnames.get(IDs.indexOf(MemID)) + " " + Snames.get(IDs.indexOf(MemID)));
            UserID.setText("User ID: " + MemID);

            bT.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    addMember(UniqueList.get(i));
    ...
    public void addMember(String MemID){
        if (NewPaymentActivity.PaymentMemberIDs.contains(MemID)){
    ...
    public class JSONTask extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            if (IDs.size()>0){
                for (String item : IDs) {
                    if (NewPaymentActivity.PaymentMemberIDs.contains(item)) {
                    } else {
                        UniqueList.add(item);
                    }
                }
